...
- 水果游戏改为“容量3的LIFO栈 + TOP相邻二消”决策：3槽只能匹配TOP，2槽只允许TOP匹配或已锁定完整A+A，0/1槽无对子时允许安全压栈继续推进。
- 修复实机日志中“成功消一对后，0槽因为暂时没有可直接A+A就CLEAN停止”的逻辑漏洞；现在会选择低风险单水果压栈并重新规划。
- 槽位视觉闭环后不再每一对都执行慢OCR；每3次二消或接近关尾时强校验一次剩余数，保留错误闭环保护。
- 水果分析宽度由720降至600，检测仅读取棋盘ROI像素；截图临时文件优先使用内部cache，降低FUSE和内存搬运开销。
- OCR分析图宽度限制为1080，并将识别框映射回原屏幕坐标；页面点击坐标仍保持原分辨率。
- 闲鱼币主页新增新版文案特征，减少COIN_HOME被误判UNKNOWN导致的重复OCR等待。
- 缩短水果点击后的固定等待，改为依靠槽位观察重试确认动画完成。
- 修复GitHub Actions仍调用缺失`offline-tests/run.sh`导致构建直接失败；Artifact更新为`xianyu-v4.40.0-debug`。
- 版本更新为 versionCode 46 / versionName 4.40.0。
