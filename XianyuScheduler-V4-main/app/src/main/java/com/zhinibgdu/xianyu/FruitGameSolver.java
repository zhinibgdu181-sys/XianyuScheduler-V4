 ...
 * V4.30 安全约束：只允许点击水果对象本身。禁止点击“打乱”“消除”
...
final class FruitGameSolver {
    ...
    static Result solveOneRound(
        ...
        while (!host.aborted()
            ...
            // V4.30: 游戏长时间无操作会自动弹出“消除/打乱/使用”道具窗口。
            // 任何情况下都不能点击道具，只关闭右上角X。
            if (handleFruitPopup(host)) {
                if (!host.sleep(500L, 800L)) return Result.ABORTED;
                continue;
            }
            ...
            host.log("[游戏V4.30] 可下落候选水果=" + objects.size()
            ...
            if (objects.size() < 10) {
                ...
                host.log("[游戏V4.30] 检测数量异常偏少，已保存视觉诊断图；本轮不会点击功能按钮");
            }
            ...
            if (pair == null) {
                ...
                host.log("[游戏V4.30] 无高置信对子；禁止点击打乱/消除/解锁/使用等功能按钮，安全停止");
                return Result.SAFE_STOP;
            }
     ...
     * 这些按钮全部禁止使用，只点击右上角关闭。
     */
    private static boolean handleFruitPopup(Host host) {
        ScreenOcr.Snapshot snap = host.ocr("水果V4.30/弹窗检测");
        if (snap == null) return false;

        String t = normalize(snap.fullText);
        boolean popup = (t.contains("打乱") || t.contains("消除"))
                && (t.contains("使用") || t.contains("开启") || t.contains("水果"));

        if (!popup) return false;

        host.log("[水果V4.30守卫] 检测到道具弹窗，禁止点击使用按钮");
        // 1440x3120设备实测右上角关闭按钮位置
        host.tap(1120, 430, "水果弹窗关闭X");
        return true;
    }
    ...
    private static List<FruitObject> detectFruitObjects(GameFrame frame) {
        ...
        // V4.30 real-device rule: fruit clipped/packed against the top cannot reliably
