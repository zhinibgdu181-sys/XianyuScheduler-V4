            ...
            try {
                ...
                if (trayChoice == null && pair == null && safePush == null) {
                    ...
                    if (tray.count >= 2) {
                        ...
                        // V4.81: 证据计数必须在“本次无动作复核”之后立即决定是否重开。
                        // 旧版先执行重开判断、后才++，导致永远只能看到上一轮的计数；
                        // 同时该分支随后直接 return，第二次证据根本不会进入同一局。
                        if (deadlockEvidenceCountV480 >= deadlockConfirmationsRequired) {
                            long nowV481 = SystemClock.elapsedRealtime();
                            if (nowV481 - lastDeadlockRestartAttemptAtV480 >= 5000L) {
                                lastDeadlockRestartAttemptAtV480 = nowV481;
                                host.log("[死局证据V4.81] 阈值已达到 "
                                        + deadlockEvidenceCountV480 + "/" + deadlockConfirmationsRequired
                                        + "，立即尝试重开");
                                RestartResult restart = restartDeadlockedRound(
                                        host, checkpoint, deadlockRestarts);
                                if (restart == RestartResult.ABORTED) return Result.ABORTED;
                                if (restart == RestartResult.RESTARTED) {
                                    deadlockRestarts++;
                                    remaining = -1;
                                    ...
                                    blockedPositionTtl.clear();
                                    deadlockEvidenceCountV480 = 0;
                                    host.log("[死局重开V4.81] ✅ 已开始新局，清空旧棋盘坐标并立即重建模型");
                                    continue;
                                }
                            }
                        }
