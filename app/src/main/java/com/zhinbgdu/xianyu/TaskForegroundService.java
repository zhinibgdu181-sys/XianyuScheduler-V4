...
public class TaskForegroundService extends Service {
    ...
    private void finishAfterTaskExecutionV466() {
        ...
        cancelCompletionNotificationV472();
        playTaskCompletionSoundV472();
        stopSelf();
    }
    ...
    private void cancelCompletionNotificationV472() {
        try {
            NotificationManager manager = getSystemService(NotificationManager.class);
    ...
    private void playTaskCompletionSoundV472() {
        ToneGenerator tone = null;
        try {
            tone = new ToneGenerator(AudioManager.STREAM_NOTIFICATION, 85);
